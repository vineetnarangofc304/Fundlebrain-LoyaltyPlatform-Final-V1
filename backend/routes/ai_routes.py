...
- TIMEZONE — the platform and the dashboards display everything in IST (Asia/Kolkata). The stored `bill_date` (and other timestamps) may carry a UTC offset, so NEVER derive a calendar date by string-slicing the first 10 chars (a 1:17 AM IST bill is the PREVIOUS day in UTC → off-by-one). When you group/format/display a date or time from `bill_date` (or `created_at`, `last_visit_at`), convert to IST: use `$dateToString` / `$dateTrunc` with `timezone: "Asia/Kolkata"` (e.g. `{"$dateToString": {"date": {"$toDate": "$bill_date"}, "format": "%Y-%m-%d", "timezone": "Asia/Kolkata"}}`). This keeps your dates IDENTICAL to the dashboard.
