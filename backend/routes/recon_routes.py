...
async def recon_init(body: ReconInitIn, user: dict = Depends(get_current_user)):
    ...
    doc = {
        ...
        "processed": 0, "queued_at": now, "queued_by": user["email"],
        "report": None,
    }
...
async def recon_finalize(body: ReconFinalizeIn, user: dict = Depends(get_current_user)):
    ...
    await recon_jobs_col.update_one({"id": body.job_id}, {"$set": {"status": "running"}})
...
async def _run_recon_job(job_id: str):
    try:
        ...
        if dataset == "transactions":
            report = await _recon_transactions(job_id, text)
        elif dataset == "customers":
            report = await _recon_customers(job_id, text)
        else:
            report = await _recon_items(job_id, text)

        await recon_jobs_col.update_one(
            {"id": job_id},
            {"$set": {"status": "done", "report": report,
                      "finished_at": datetime.now(timezone.utc).isoformat()}})
    except Exception as e:
...
async def _progress(job_id: str, processed: int):
    await recon_jobs_col.update_one({"id": job_id}, {"$set": {"processed": processed}})


async def _recon_transactions(job_id: str, text: str) -> Dict[str, Any]:
    ...
    for r in reader:
        ...
        if total_rows % 50000 == 0:
            await _progress(job_id, total_rows)
    ...
    # ---- DB → CSV pass: bills in DB that are NOT in the CSV ----
    extra_count = 0
    extra_sample = []
...
async def _recon_customers(job_id: str, text: str) -> Dict[str, Any]:
    reader = csv.DictReader(io.StringIO(text))
...
async def _recon_items(job_id: str, text: str) -> Dict[str, Any]:
    items_col = db["items"]
    reader = csv.DictReader(io.StringIO(text))
    csv_keys: set = set()
    total_rows = parse_failed = dup_keys = 0
